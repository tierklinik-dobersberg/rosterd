...
func LoadHolidays(ctx context.Context, country string, year int) ([]PublicHoliday, error) {
	req, err := retryablehttp.NewRequestWithContext(ctx, "GET", apiURL(country, year), nil)
	if err != nil {
		return nil, err
	}
...
func NewHolidayCache(log hclog.Logger) *HolidayCache {
	return &HolidayCache{
...
func (cache *HolidayCache) Get(ctx context.Context, country string, year int) ([]PublicHoliday, error) {
	...
	if entry, ok := cache.cache[fmt.Sprintf("%s-%d", country, year)]; ok {
		...
		cache.logger.Debug("Using cache entry for holidays", "country", country, "year", year)

		if entry.Loaded.Before(time.Now().Add(time.Hour * -24)) {
			cache.logger.Info("Re-fetching holidays", "country", country, "year", year)
			go func() {
				_, err := cache.load(country, year)
				if err != nil {
					cache.logger.Error("failed to re-fetch holidays", "country", country, "year", year, "error", err)
				}
			}()
		}
	...
	cache.logger.Info("Fetching holidays", "country", country, "year", year)
